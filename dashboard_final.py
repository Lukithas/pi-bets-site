...
st.sidebar.image(
    "https://institucional.uniceub.br/hubfs/BrandCenter/img/logo-ceub-versao-estendida.png", use_container_width=True)
st.sidebar.title("Configurações")

# Limpa memória e usa estilo base transparente
plt.close('all')

# CSS focado apenas na estrutura dos Cards, deixando as cores para o Streamlit (theme="streamlit")
st.markdown('''
<style>
    .grafico-desc { 
        padding: 12px 16px; 
...
if sus_encontrado:
    st.sidebar.success("✅ dados_sus.csv carregado!")
else:
    st.sidebar.warning("⚠️ SUS (CSV) não encontrado. Usando Mockup.")
if csv_encontrado:
    st.sidebar.success("✅ dados_apostas.csv carregado!")
else:
    st.sidebar.warning("⚠️ CSV não encontrado. Usando Mockup.")
...
with tab1:
    def render_card(col, title, label, value, fig, desc):
        with col:
            with st.container(border=True):
                ...
                fig.tight_layout()
                # O SEGREDO AQUI: theme="streamlit" faz o gráfico acompanhar o dark mode do menu!
                st.pyplot(fig, use_container_width=True, theme="streamlit")
                st.markdown(
    ...
    render_card(col1, "1. Modalidades", "Liderança", "71.3%", fig1,
                "<strong>Análise:</strong> Loterias dominam.")

    fig2, ax2 = plt.subplots(figsize=(6, 3))
    sns.lineplot(data=df_bcb, x='Ano', y='Inadimplencia',
                 ax=ax2, color='#ef4444', marker='o')
    render_card(col2, "2. Inadimplência", "Média Atual", "3.1%", fig2,
                "<strong>Análise:</strong> Acompanha o crescimento BETs.")
    ...
    if not df_pacientes.empty:
        ...
        render_card(col3, "3. Idade", "Média",
                    f"{int(df_pacientes['Idade'].mean())} anos", fig3, "<strong>Análise:</strong> Jovens adultos.")

        fig4, ax4 = plt.subplots(figsize=(6, 3))
        sns.boxplot(data=df_pacientes, x='Genero',
                    y='Divida_Acumulada', ax=ax4, palette="muted")
        render_card(col4, "4. Dívida/Género", "Impacto", "Variável",
                    fig4, "<strong>Análise:</strong> Dispersão maior em homens.")
    ...
    if not df_pacientes.empty and 'Renda_Mensal' in df_pacientes.columns:
        ...
        render_card(col5, "5. Renda vs Dívida", "Correlação", "Direta",
                    fig5, "<strong>Análise:</strong> Dívida acompanha a renda.")
    ...
    render_card(col6, "6. Reclamações", "Top", "Saque", fig6,
                "<strong>Análise:</strong> Saque lidera.")

    col7, col8 = st.columns(2)
    with col7:
        fig7, ax7 = plt.subplots(figsize=(6, 3))
        df_fin = df_consolidado[df_consolidado['Indicador'].str.contains(
            'dívida|poupança|lazer|alimentação|renda|consequência|inadimplência', case=False, na=False)].copy()
        if not df_fin.empty:
            ...
            ax7.set_ylim(0, 100)
        render_card(col7, "7. Orçamento", "Alerta", "Alto", fig7,
                    "<strong>Análise:</strong> Impacto na renda familiar.")
    ...
    with col8:
        ...
        if not df_sui.empty:
            ...
            ax8.set_ylim(0, 100)
        render_card(col8, "8. Risco Suicida", "Crescimento", "Crítico",
                    fig8, "<strong>Análise:</strong> Salto expressivo.")
...
with tab2:
    ...
    st.download_button("📥 Baixar CSV (SUS)", data=convert_csv(
        df_pacientes), file_name='dados_sus.csv', mime='text/csv')

    st.markdown("---")
    st.subheader("2. Pesquisa Equipa")
    st.dataframe(df_consolidado, use_container_width=True)
    st.download_button("📥 Baixar CSV (Equipa)", data=convert_csv(
        df_consolidado), file_name='dados_equipa.csv', mime='text/csv')
